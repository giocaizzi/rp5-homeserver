...
**`services/`.** `deploy-services.yml` runs on push touching `services/**`: a `detect` job diffs the push to find changed `services/<stack>/` dirs, then a matrix `deploy` job POSTs each stack's Portainer GitOps webhook through the CF tunnel (auth: `github-webhooks` CF Access service token + Portainer webhook-bypass policy, both in `cloud/main.tf`). Env `pi-services`; needs repo var `PORTAINER_URL`, secrets `CF_ACCESS_CLIENT_ID`/`CF_ACCESS_CLIENT_SECRET` (terraform outputs) and `PORTAINER_WEBHOOK_<STACK>` per stack. Adding a stack: add the secret **and** a `WH_<STACK>` line to the deploy job env.
