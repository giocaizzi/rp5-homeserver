...
When the volume is first created, Docker only auto-copies the image's `storage/*` contents into the volume if the volume is **empty** at mount time. The moment anything exists in the volume, Docker treats it as already-initialized and uses it as-is — the image's `app/`, `build/`, `framework/`, `logs/`, etc. are then **not** copied, and Firefly crashes on boot because Laravel can't find them.

The image itself ships no OAuth keys (they are generated by Passport at runtime), so on a normal first deploy the volume gets populated cleanly with everything *except* the keys, Passport then writes fresh keys into it, and **every existing PAT is invalidated**. Two paths:
...
2. **Preserve current PAT — pre-seed the volume.** The pre-seed must be **all-or-nothing**: the volume needs both the full Laravel `storage/` tree from the image *and* the live OAuth keys before the new container ever mounts it. Otherwise the volume is "partly initialized" and Firefly crashes (see paragraph above).

   ```bash
   ssh pi@pi.local '
     # 1. Capture the live OAuth keys from the running container.
     APP=$(docker ps -qf "label=com.docker.swarm.service.name=firefly_app" | head -1)
     docker cp "$APP":/var/www/html/storage/oauth-private.key /tmp/op.key
     docker cp "$APP":/var/www/html/storage/oauth-public.key  /tmp/oP.key

     # 2. Create the volume.
     docker volume create firefly_firefly_storage

     # 3. Populate with the FULL storage tree from the image, then overlay the
     #    captured keys. --entrypoint sh bypasses the Firefly bootstrap (which
     #    would otherwise try to boot Laravel without env vars and exit 1).
     #    --user 0 gives us write perms inside /dst (image user is www-data).
     docker run --rm --entrypoint /bin/sh --user 0 \
       -v firefly_firefly_storage:/dst -v /tmp:/src \
       fireflyiii/core:latest -c "
         set -e
         for entry in app build database debugbar export framework logs upload .htaccess; do
           cp -a /var/www/html/storage/\$entry /dst/\$entry
         done
         cp /src/op.key /dst/oauth-private.key
         cp /src/oP.key /dst/oauth-public.key
         chown -R www-data:www-data /dst
         chmod 600 /dst/oauth-*.key
       "

     shred -u /tmp/op.key /tmp/oP.key
   '
   ```

   The volume now contains the same layout the image would have produced *plus* the working keys. When Portainer redeploys, Docker sees a non-empty volume → skips initialization → Firefly boots with everything in place → the existing PAT keeps working.
